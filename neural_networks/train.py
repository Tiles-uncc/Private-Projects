...
class HyperParametersAndTransforms():
    ...
    class NeuralNetworkRegressor():
        ...
        model_kwargs = dict(
            ...
            epochs = 2000,  # TODO (REQUIRED) Set neural network's  epochs
            ...
            seed = 0, # TODO (OPTIONAL) Set the neural network to random state seed
        )
        ...
        data_prep_kwargs = dict(
            ...
            use_features = ['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX', 'RM',
                            'DIS', 'RAD', 'TAX', 'LSTAT'],
        )
    ...
    class NeuralNetworkClassifier():
        ...
        model_kwargs = dict(
            neurons_per_layer = [35,35,35,10], # TODO (REQUIRED) Set neural network neurons per layer
            ...
            alpha = .15, # TODO (REQUIRED) Set neural network's learning rate
            epochs = 17,  # TODO (REQUIRED) Set neural network's  epochs
        ...
        data_prep_kwargs = dict(
            # TODO (REQUIRED) Add Pipeline() definitions below
            target_pipe = Pipeline([('one_hot', OneHotEncoding())]),
            # TODO (REQUIRED) Add Pipeline() definitions below
            feature_pipe = Pipeline([('image', ImageNormalization())]),
        )